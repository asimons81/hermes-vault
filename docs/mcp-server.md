...
**Arguments:**
- `agent_id` (required) --- Identity for policy enforcement
...
- A paired `oauth_refresh_token` (alias `refresh`) must exist in the vault
...
| "No refresh token found" | No `oauth_refresh_token` alias `refresh` stored | Run `oauth_login` first |
