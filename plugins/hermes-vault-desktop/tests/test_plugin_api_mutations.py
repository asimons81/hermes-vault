...
def test_add_route(client, fake_popen, clean_env, mutations_on):
    ...
    assert proc.args == ["hermes-vault", "--no-banner", "desktop-bridge", "--allow-mutations"]
...
def test_argv_allow_mutations_only_for_mutations(client, fake_popen, clean_env, mutations_on):
    ...
    assert fake_popen.instances[-1].args == ["hermes-vault", "--no-banner", "desktop-bridge", "--allow-mutations"]

    # GET route: argv unchanged.
    fake_popen.stdout = _ok_result({"profile": "default"})
    client.get("/overview")
    assert fake_popen.instances[-1].args == ["hermes-vault", "--no-banner", "desktop-bridge"]
